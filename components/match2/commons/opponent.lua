...
function Opponent.readOpponentArgs(args)
	...
	elseif args.type == 'literal' then
		return {type = 'literal', name = args.name or args[1] or ''}
...
function Opponent.fromMatch2Record(record)
	...
	elseif record.type == 'literal' then
		return {type = 'literal', name = record.name or ''}
