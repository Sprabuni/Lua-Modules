...
function CustomMatchGroupInput.processOpponent(record, date)
	...
	if opponent.type == 'team' and opponent.template:lower() == 'bye' then
		opponent = {type = 'literal', name = 'BYE'}
	end

	Opponent.resolve(opponent, date)
	return MatchGroupInput.mergeRecordWithOpponent(record, opponent)
end
...
function matchFunctions.getOpponents(args)
	...
	for opponentIndex = 1, MAX_NUM_OPPONENTS do
		...
		if not Logic.isEmpty(opponent) then
			opponent = CustomMatchGroupInput.processOpponent(opponent, args.date)
